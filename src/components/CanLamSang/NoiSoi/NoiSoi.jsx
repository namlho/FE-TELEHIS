...
export default function NoiSoi() {
    ...
    try {
      ...
      for (const row of data) {
        ...
        if (!map.has(pt.id_patient)) {
          map.set(pt.id_patient, {
            ...
            address: pt.address,
            created_at: pt.created_at || pt.createdAt || pt.uploaded_at_vn || (Array.isArray(pt.image_storage) && (pt.image_storage[0]?.uploaded_at_vn || pt.image_storage[0]?.uploaded_at)) || pt.uploaded_at || null,
          });
        }
      }
      ...
      try {
        ...
        for (const pt of pList) {
          if (!pt?.id_patient) continue;
          const hasEndoPrimary = pt.primary_disease_code === DISEASE_CODE;
          const hasEndoInStorage = Array.isArray(pt.image_storage)
            && pt.image_storage.some(s => s.disease_code === DISEASE_CODE || s.diseases?.code === DISEASE_CODE);
          if (hasEndoPrimary || hasEndoInStorage) {
            ...
            if (!map.has(pt.id_patient)) {
              map.set(pt.id_patient, {
                ...
                primary_disease_code: pt.primary_disease_code || null,
              });
            } else {
              const existing = map.get(pt.id_patient) || {};
              const updated = { ...existing };
              if (!updated.created_at && fallbackCreated) updated.created_at = fallbackCreated;
              if ((!updated.address || updated.address === "") && pt.address) updated.address = pt.address;
              if ((!updated.full_name || updated.full_name === "") && pt.full_name) updated.full_name = pt.full_name;
              if ((!updated.dob || updated.dob === "") && pt.dob) updated.dob = pt.dob;
              if ((!updated.gender || updated.gender === "") && pt.gender) updated.gender = pt.gender;
              if ((!updated.phone_number || updated.phone_number === "") && pt.phone_number) updated.phone_number = pt.phone_number;
              updated.primary_disease_code = updated.primary_disease_code || pt.primary_disease_code || null;
              map.set(pt.id_patient, updated);
            }
          }
        }
      } catch (errP) {
      ...
      // debug: log created_at sample for verification
      // eslint-disable-next-line no-console
      console.log('[ENDO] tableData created_at sample:', tableData.slice(0, 20).map(t => ({ id: t.id_patient, created_at: t.created_at })));
      if (searchId) {
      ...
      // make pagination reflect number of unique patient rows (deduped)
  ...
  useEffect(() => {
    debouncedSearch();
  }, [searchId, fromDate, toDate, searchKeyword]); // eslint-disable-line
  ...
  const handleUpload = async ({ file, onSuccess, onError }) => {
    ...
    try {
      ...
      const msg = err?.response ? `Tải ảnh thất bại: ${err.response.status} ${err.response.data?.error || err.response.data?.message || ""}` : "Tải ảnh thất bại";
      message.error(msg);
      onError?.(err);
    }
  };
  ...
  const handleDeleteImage = (img) => {
    ...
    Modal.confirm({
      ...
      content: `Bạn có chắc muốn xóa ảnh "${filename}" của BN ${idp || ""}?`,
      onOk: async () => {
        try {
          if (img.id) {
          ...
          const msg = err?.response ? `Không thể xóa ảnh: ${err.response.status} ${err.response.data?.error || err.response.data?.message || ""}` : "Không thể xóa ảnh";
          message.error(msg);
        }
      }
    });
