    ...
    try {
      ...
      try {
        ...
        for (const pItem of pList) {
          ...
          const hasMSKPrimary = pItem.primary_disease_code === DISEASE_CODE;
          const hasMSKInStorage = Array.isArray(pItem.image_storage)
            && pItem.image_storage.some(s => s.disease_code === DISEASE_CODE || s.diseases?.code === DISEASE_CODE);
          if (hasMSKPrimary || hasMSKInStorage) {
      ...
      // debug: log created_at sample to verify merge
      // eslint-disable-next-line no-console
      console.log('[MSK] tableData created_at sample:', tableData.slice(0, 20).map(t => ({ id: t.id_patient, created_at: t.created_at })));
      if (searchId) {
      ...
      // Use the number of unique patient rows as the total shown in pagination.
      // imagesAPI.list may return image-level totals while we deduplicate by patient,
      // so setTotal to tableData.length to avoid the UI showing a larger total than rows.
