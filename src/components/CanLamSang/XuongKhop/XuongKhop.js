...
const DISEASE_CODE = 'MSK'; // Xương khớp (musculoskeletal)
...
export default function XuongKhop() {
    ...
    const fetchPatients = useCallback(async () => {
        ...
        try {
            ...
            console.debug('[MSK] fetchPatients - data length:', Array.isArray(data) ? data.length : 0);
            ...
            // Client-side exact id search fallback
            if (searchId) {
                const sid = String(searchId).trim();
                tableData = tableData.filter(item => String(item.id_patient) === sid);
                console.debug('[MSK] Applied client-side id filter, remaining rows:', tableData.length, 'searchId:', sid);
            }
            if (searchKeyword) {
    ...
    const inferExtensionFromBase64 = (base64, mime) => {
        ...
        if (head.startsWith('SUkq')) return '.tif';
        if (head.startsWith('UklGR')) return '.webp';
        return '';
    };

    // Sanitize filename and ensure an extension exists
