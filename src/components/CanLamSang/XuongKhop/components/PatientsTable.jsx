...
export default function PatientsTable({
  ...
  const columns = [
    ...
    {
      title: 'Ngày tạo', dataIndex: 'created_at', key: 'created_at', width: 200,
      render: (val, record) => {
        const dateVal = val || record?.createdAt || record?.created || record?.created_at;
        if (!dateVal) return '';
        const parsed = dayjs(dateVal);
        return parsed.isValid() ? parsed.format('HH:mm DD/MM/YYYY') : String(dateVal);
      }
    },
    {
